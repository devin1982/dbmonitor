                ...
                if ($query_slowquery->num_rows() > 0)
		        {
                     ...
                     foreach($slowquery as $record){
                      ...
                      $uri='';
				 	            ...
				 	            $content =
				 	            $table_content.
				 	            "<br> <font color='#FF0000'> 该邮件由系统自动发出，请勿回复，语句详细执行情况请登录 http://10.2.0.100:8888/anemometer 通过邮件中checksum值搜索.</font>";
			
                      $send_mail_to_list = array_filter(explode(';',$send_mail_to_list));
                                $mail_body = $content;
                                $this->load->library('mailer');
				 	            if( ! $this->mailer->sendmail($send_mail_to_list , $title, $mail_body) ){
                         ...
                         echo $server." Send slowquery mail success!</br>";
                      }
                }
                 else{
